...
//		}
        ...
        if (obje != null) {
            System.out.println("Person has multimedia object:" + obje);
            ...
            System.out.println("content=" + obje.getText());
        }
    }
    ...
    public IndividualJDOM(Individual newIndividual) {
        if (newIndividual instanceof IndividualJDOM) {
            ...
            setFullName(newIndividual.getNamePrefix(), newIndividual.getGivenNames(), newIndividual.getSurname(), newIndividual.getNameSuffix());
...
//		name += surname;
        ...
        System.out.println("name before slashes removed:|"+name+"|");
        name = name.replaceAll("/", "");
        System.out.println("name  after slashes removed:|"+name+"|");
        return name.trim();
    }
...
//			saveName();
    ...
    public void setFullName(String name) {
      System.out.println("--------------------------IndividualJDOM.setFullName(name):"+name);
        if (name == null) {
            name = "";
        }
        if (name.equals(getFullName())) {
          System.out.println("IndividualJDOM.setFullName(name) name is already same as full name, returning...");
          return;
        }
        int firstSlashIndex = name.indexOf("/");
        int lastSlashIndex = name.lastIndexOf("/");//, firstSlashIndex + 1);
        System.err.println("name:"+name);
        System.err.println("surname:"+ firstSlashIndex + "-" + lastSlashIndex);
...
//            element.removeChildren(NAME);
    ...
    private void saveName() {
        ...
        name += getNameSuffix();
        System.out.println("saveName name="+name.trim());
        setName(name.trim());
    }
...
//		familiesAsSpouse.add(family);
    ...
    public Gender getGender() {
    	System.out.println("IndividualJDOM.getGender() returns:"+element.getChildTextTrim(GENDER));
...
// todo implement
    ...
    public Family getFamilyAsChild() {
		...
		try {
            ...
            System.out.println("famNode: "+famNode);
            try {
    ...
    public Family getFamilyAsSpouse() {
		...
		try {
            ...
            if (famLink != null) {
			XPath xpath = XPath.newInstance("//FAM[@ID='"+famLink.getAttributeValue(REF)+"']");
            System.out.println("spouse xpath:"+xpath.getXPath());
            Element famNode = (Element) xpath.selectSingleNode(element);
            System.out.println("famNode: "+famNode);
...
//    System.out.println("IndividualJDOM.getEvents()");
    if (events == null) {
      System.out.println("IndividualJDOM.getEvents() creating event list");
      events = new ArrayList();
      try {
        XPath xpath = XPath.newInstance(eventNodeNames);
        System.out.println("xpath = "+xpath);
        List eventNodes = xpath.selectNodes(element);//, eventNodeNames);
        System.out.println("XPath events: " + eventNodes.size());
