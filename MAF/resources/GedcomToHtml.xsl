...
<xsl:template match="INDI">
    ...
    <h1><a name="#{@ID}"><xsl:apply-templates select="NAME"/></a></h1>
    ...
    <table width="100%" bgcolor="blue"><tr><td><xsl:text>&#xa0;</xsl:text></td></tr></table>
...
<xsl:template match="NAME">
    ...
    <xsl:when test="string-length(.) = 2 + string-length(translate(., '/', ''))">
        ...
        <xsl:text> </xsl:text>
        <b><u><xsl:value-of select="substring-before(substring-after(.,'/'), '/')"/></u></b>
        <xsl:text> </xsl:text>
