...
import com.apple.cocoa.foundation.NSMutableRange;
...
//   private NSMutableAttributedString nameText;
   ...
   private static final NSDictionary times10BoldDict = new NSDictionary(times10Bold, NSAttributedString.FontAttributeName);
   private static final NSDictionary times12Dict = new NSDictionary(times12, NSAttributedString.FontAttributeName);
   private static final NSDictionary times12BoldDict = new NSDictionary(times12Bold, NSAttributedString.FontAttributeName);
   ...
   public FamilyGroupSheetView(NSRect frame, Individual individual) {
           super(frame);
         System.out.println("FamilyGroupSheetView(frame="+frame+" indi="+individual.getFullName());
   ...
   public boolean knowsPageRange(NSMutableRange nsMutableRange) {
      System.out.println("knowsPageRange(range="+nsMutableRange+")");
      final int numChildren = individual.getFamilyAsSpouse().getChildren().size();
      if (numChildren > 5) {
         nsMutableRange.setLength(1 + numChildren / 5);
         System.out.println("pagerange set to "+nsMutableRange+" for "+numChildren+" children.");
         return true;
      }
   ...
   public NSRect rectForPage(int i) {
      System.out.println("rectForPage(page#="+i+")");
      ...
      System.out.println("rectForPage rect="+pageRect);
      return pageRect;
   }

//   public void drawPageBorderWithSize(NSSize nsSize) {
//      System.out.println("drawPageBorderWithSize(size="+nsSize+")");
...
//   }
   ...
   public void drawRect(NSRect rect) {
           // Drawing code here.
          System.out.println("FamilyGroupSheetView.drawRect(rect="+rect+")");
      ...
      float y = 0;
          try {
             ...
             final NSAttributedString titleStr = new NSAttributedString("Family Group Record", (NSDictionary)centeredTitleDict);
             ...
             NSGraphics.drawAttributedString(new NSAttributedString(new NSGregorianDateFormatter("%a, %b %e, %Y", false).stringForObjectValue(new NSGregorianDate()), LABEL_FONT), rect);
...
//             y += NSGraphics.sizeOfAttributedString(titleStr).height();
                final Individual husband = individual.getFamilyAsSpouse().getFather();
                final NSMutableAttributedString husbandStr = new NSMutableAttributedString("Husband: "/*+husband.getFullName()*/, LABEL_FONT);
             ...
             final Individual wife = individual.getFamilyAsSpouse().getMother();
...
//                 childHeader += "M/F\t\t\t\tin order of birth\t\t\t\t\t\tfor Children";
             ...
             int num = 1;
             for (Iterator iterator = individual.getFamilyAsSpouse().getChildren().iterator(); iterator.hasNext() && num <= 5;) {
                ...
                spouseStr.appendAttributedString(new NSAttributedString(child.getPrimarySpouse().getFullName(), DATA_FONT));
...
//             y += NSGraphics.sizeOfAttributedString(titleStr).height();
                ...
                parentYearStr.appendAttributedString(new NSAttributedString(""+cal.get(GregorianCalendar.YEAR), DATA_BOLD_FONT));
                ...
                List childrenForPage =  individual.getFamilyAsSpouse().getChildren().subList(num-1, Math.min(individual.getFamilyAsSpouse().getChildren().size(), num-1+7));
                 String childHeader = "Sex Children\nM/F";
                String childHeader2 = "List each child (living or dead)\nin order of birth";
                String childHeader3 = "LDS Ordinance Data\nfor Children";
//                 childHeader += "M/F\t\t\t\tin order of birth\t\t\t\t\t\tfor Children";
   ...
   private float drawIndividualDetailInRect(Individual individual, NSRect rect, boolean isChild) {
      System.out.println("drawIndividualDetailInRect("+individual+", "+rect+")");
      if (individual == null) individual = new Indi();
...
//      NSGraphics.frameRectWithWidth(rect, 3);
      ...
      NSGraphics.drawAttributedString(makeIndividualOrdinanceString(individual, isChild), rect.rectByOffsettingRect(300, 0));
   ...
   private NSAttributedString makeIndividualPlaceString(Individual individual, boolean isChild) {
      ...
      if (isChild || individual.getGender().equals(Gender.MALE)) {
         ...
         if (marriageEvent == null) {marriageEvent = new MyEvent();}
         detailStr.appendAttributedString(new NSAttributedString("\nPlace: ", LABEL_FONT));
         detailStr.appendAttributedString(new NSAttributedString(marriageEvent.getPlace().getFormatString()+" ", DATA_FONT));
      }
      return detailStr;
   }
   ...
   private NSAttributedString makeIndividualOrdinanceString(Individual individual, boolean isChild) {
      ...
      detailStr.appendAttributedString(new NSAttributedString(baptismEvent.getDateString() + "  "+baptismEvent.getTemple().getCode()+"\n", DATA_FONT));
      ...
      detailStr.appendAttributedString(new NSAttributedString(endowmentEvent.getDateString() + "  "+endowmentEvent.getTemple().getCode(), DATA_FONT));
      ...
      detailStr.appendAttributedString(new NSAttributedString("\nSlg P: ", LABEL_FONT));
      detailStr.appendAttributedString(new NSAttributedString(sealingParentsEvent.getDateString() + "  "+sealingParentsEvent.getTemple().getCode(), DATA_FONT));
      if (isChild || individual.getGender().equals(Gender.MALE)/* || currColor++%2==0*/) {
         Ordinance sealingSpouseEvent = individual.getFamilyAsSpouse().getSealingToSpouse();
         if (sealingSpouseEvent == null) {sealingSpouseEvent = new MyOrdinance();}
         detailStr.appendAttributedString(new NSAttributedString("\n\nSlg S: ", LABEL_FONT));
         detailStr.appendAttributedString(new NSAttributedString(sealingSpouseEvent.getDateString() + "  "+sealingSpouseEvent.getTemple().getCode(), DATA_FONT));
      }
      return detailStr;
   }
