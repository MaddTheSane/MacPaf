...
// changed '\n' to line.separator for cross platform compatibility
...
public class GedcomOutputter extends DefaultHandler {
  ...
  Writer writer;
  // for compatibility with DOS systems and old PAF for Mac 2.3.1
  ...
  public void startDocument() throws SAXException {
	...
	try {
	  writer = new AnselOutputStreamWriter(outStream);
	}
	catch (Exception err) {
  ...
  private class GedcomLine {
	...
	public void write() throws SAXException {
	  try {
		...
		writer.write(text.toString() + lineSeparator);
	  }
	  catch (java.io.IOException err) {
		throw new SAXException(err);
	  }
